...
Convert comma separated text strings into charts image JPG
...
my %_chart_formats = (
    ...
    bars        => 'GD::Graph::bars',
    hbars       => 'GD::Graph::hbars',
    bars3d      => 'GD::Graph::bars3d',
    lines3d     => 'GD::Graph::lines3d',
    pie         => 'GD::Graph::pie3d',
) ;
...
create a simple chart image, with some nice defaults
 ...
 parameters
    ...
    hashref params of
        ...
        columns - columns to be included in the chart "1,2,3,4"
...
sub process
{
    ...
    if ( !-f $filename ) {
        ...
        @data = split_csv_data( $content, $params->{separator} ) ;
        ...
        $chart->set(
            dclrs => [
                qw(marine blue lred dgreen orange salmon lbrown gold lgreen yellow gray dred lpurple)
            ]
        ) ;
        ...
        my ( $stdout, $stderr, $exit ) = capture {
            ...
            path($filename)->spew_raw( $gd->jpeg ) if ($$gd) ;
        } ;
    }
